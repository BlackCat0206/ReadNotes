...
* 二分法的核心：结束后的区域[left, nums.size()) 是满足向右收束（left++）条件的区间；

**74_搜索二维矩阵**：

* 有序矩阵：使用二分法将`l = 0; r  = m * n - 1; int x = mid / n; int y = mid % n`

**34_在排序数组中查找元素的第一个和最后一个位置**：

* 实现`lower_bound`：向右收束条件`(nums[mid] < target)`，即`[left, nums.size())`区间满足所有元素均`>=targetNum`；
* 实现`upper_bound`：向右收束条件`(nums[mid] <= target)`，即`[left, nums.size())`区间满足所有元素均`>targetNum`;

**33_搜索旋转排序数组**：

* 找到旋转的开始下标`index`：二分法用`nums.back()`作为`target`，向右收束条件`(nums[mid] > target)`,即`[left, nums.size())`区间满足所有元素均`<= nums.back()`;
* 根据`nums.back()`与`target`进行比较：
  * 小于`target`：搜索范围`[index, nums.size() - 1]`;
  * 大于`target`：搜索范围`[0, index - 1]`;
...
* **如何划分两个数组**：`(m + n + 1) / 2`，默认第一个组比第二组多一个

  * 偶数：`(m + n) / 2`
  * 奇数：`(m + n + 1) / 2`
  * 整合：`(m + n + 1) / 2`

* **对于入参的判断**：满足`nums1.size() <= nums2.size()`；

  * 满足`nums1.size() <= nums2.size()`；如果`m > n`，无法从`i=0`开始枚举。以`m=5,n=3`, `i = 0`时，`b`数组需要有4个数在第一组，但`n = 3 < 4`,无法做到。保证`m <= n`可以让我们从`i = 0`开始枚举，写起来更方便

* **核心状态方程（注意理解演化过程）**：

  * `max(a[i], b[j]) <= min(a[i + 1], b[j + 1]);`
  * `(a[i] <= b[j + 1] && a[i + 1] >= b[j])`
  * i越小`a[i] <= b[j + 1]`越容易成立，通过**二分找到最大的能满足条件**的即可。

* **`i`的含义以及初始化**：

  * `i`：表示`nums1`有**几个元素**划分在**比较数组**中;
  * **初始化**：`i = l + (r - l) / 2`;

* **`j`的含义以及初始化**：

  * j：表示`nums2`有**几个元素**划分在**比较数组**中;
  * **初始化**：`j = (m + n  + 1) / 2 - i`;

* **二分法边界和区间**：

  * **区间**：开区间；
  * **边界**：`l = -1, r = m`;  表示比较数组中有几个`nums1`的元素
  * `i`的含义：比较数组中有`i + 1`个`nums1`中的元素；
  * **`j`的计算：`j = (m + n + 1)  / 2 - (i + 1) - 1;`**
    * (j + 1) = (m + n + 1) / 2 - (i + 1);
    * `j = (m + n + 1)  / 2 - (i + 1) - 1;`
