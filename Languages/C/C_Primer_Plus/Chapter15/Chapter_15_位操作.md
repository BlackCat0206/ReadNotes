...
掩码（*mask*）：常用于检查位或控制开关（例：一些传感器就根据其传输数据中的固定位的0或1来控制传感器）。
...
flags &= ~(MASK);	// 结果是：0000 0000
```

#### 2.2.3 切换位：异或`^`

* 切换位指打开以关闭的位，和关闭已经打开的位置。

```c
flags ^= MASK;

// 假设 (00000111) ^ (10110110)
// 其结果是：(10111001)
```
...
* 左移：运算对象* 2^N（N是位移个数）。
* 右移：运算对象/ 2^N （**向下取整**：**计算结果取整不能大于计算结果**）。

### 2.4 位字段

位字段（*bit field*）：是一个`unsigned int`和`signed int`相邻的组合。需要由一个结构体声明，需要一个变量的标识符，并确认它的位宽。

```c
struct {
    unsigned int autfd : 1;
    unsigned int bldfc : 1;
    unsigned int elsle : 1;
    unsigend int itals : 1;
}
```
...
本章主要介绍了基本的逻辑运算符，其中位移运算符的左移与右移的区别在开发中要多注意。（左移不区分符号，空白填0。右移区分符号位，空白位由符号位填充。）