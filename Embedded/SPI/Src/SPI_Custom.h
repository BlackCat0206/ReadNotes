...
typedef struct{
    uint32_t ECSPIx_RXDATA;        // 0x00 接收数据寄存器
    uint32_t ECSPIx_TXDATA;        // 0x04 发送数据
    uint32_t ECSPIx_CONREG;        // 0x08 控制寄存器
    uint32_t ECSPIx_CONFIGREG;     // 0x0C 配置寄存器
    uint32_t ECSPIx_INTREG;        // 0x10 中断寄存器
    uint32_t ECSPIx_DMAREG;        // 0x14 DMA 控制寄存器
    uint32_t ECSPIx_STATREG;       // 0x18 状态寄   存器
    uint32_t ECSPIx_PERIODREG;     // 0x1C 时钟分频寄存器
    uint32_t ECSPIx_TESTREG;       // 0x20 测试寄存器
    uint32_t ECSPIx_MSGDATA;       // 0x24 消息数据寄存器
}SPI_Register;
