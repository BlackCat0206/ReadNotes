...
**逻辑真值表**：Z表示高阻态(1)，OUTB1、OUTA 不能同时输出1，用**高阻态**代替了。
...
**出现原因**：物理按键按下或松开的瞬间，由于弹簧的原因，会产生几毫米到几十毫秒的电平抖动，如果直接读取电平会导致误触发。消抖的核心就是**过滤掉这段抖动时间的不稳定信号**，只识别稳定的按键。

**消抖方式**：“**中断触发 -> 定时器延时验证-> 执行逻辑**”，不在中断函数中做阻塞和延时操作。
...
{
    // 创建消抖定时器
    g_key1_debounce_timer = xTimerCreate("Key1Debounce", pdMS_TO_TICKS(10), pdFALSE, (void*)0, Key1_DebounceCallback);
    g_key2_debounce_timer = xTimerCreate("Key2Debounce", pdMS_TO_TICKS(10), pdFALSE, (void*)0, Key2_DebounceCallback);    
}
...
{
    // 清除中断标志
    GPIO_ClearPinsInterruptFlags(KEY2_GPIO, 1U << KEY2_PIN);
    PRINTF("Key 2 Pressed!\r\n");
    BaseType_t xHigherPriorityTaskWoken = pdFALSE;
    // 重启消抖定时器
    if (g_key2_debounce_timer){
        xTimerResetFromISR(g_key2_debounce_timer, &xHigherPriorityTaskWoken);
        portYIELD_FROM_ISR(xHigherPriorityTaskWoken);
    }
}

/**
 * @brief 按键消抖Key1定时器回调函数
 * @param [in] xTimer 定时器句柄
 * @return void
 * @details
 * 定时器到期后检查按键状态，
 */
void Key1_DebounceCallback(TimerHandle_t xTimer)
{
    PRINTF("Key 1 Debounce Check\r\n");
    if (Key1_Read() == KEY_IS_PRESSED)
    {
        // 发送电机启动命令到队列
        if (g_motor_cmd_queue)
        {
            motor_cmd_t cmd = MOTOR_CMD_START;
            xQueueSend(g_motor_cmd_queue, &cmd, 0);
        }

        // 发送OLED显示欢迎信息命令到队列
        if (g_oled_cmd_queue)
        {
            oled_cmd_t cmd = OLED_CMD_WELCOME;
            xQueueSend(g_oled_cmd_queue, &cmd, 0);
        }
    }
}
```
